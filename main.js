import { gsap } from 'gsap'
import { navbar } from './navbar'
import { car } from './car'
import './style.css'

window.onload = () => {
	navbar.adjust()
	loadAnimation()
}

window.onresize = () => {
	navbar.adjust()
}

function loadAnimation() {
	const tween = gsap.timeline({ delay: 0.8 })

	// 设置 loaing 动画
	tween.to('.loading', { direction: 0.5, opacity: 0, display: 'none' })

	// 设置 header 动画
	tween.from('header', { direction: 0.5, ease: 'power2', y: -16 * 6 })
	tween.to('header', { direction: 0, transition: 0.3 })

	// 设置 aside 动画
	tween.from('.info', { direction: 0.5, ease: 'power2', opacity: 0, y: 40 })

	// 设置 svg 动画
	tween.from('.background-svg', {
		direction: 1.5,
		ease: 'power2',
		opacity: 0,
		x: 80
	})

	tween.call(carAnimation)
}

function carAnimation() {
	car.play()
}

const navIcon = document.querySelector('header i')
navIcon.addEventListener('click', () => {
	gsap.fromTo(
		'.menu',
		{ y: -window.innerHeight, opacity: 0, display: 'none' },
		{ duration: 1.2, ease: 'power2', opacity: 1, x: 0, y: 0, display: 'flex' }
	)
})

const menuIcon = document.querySelector('.menu i')
menuIcon.addEventListener('click', () => {
	const tweem = gsap.timeline()
	tweem.to('.menu', { duration: 0.3, y: 30 })
	tweem.to('.menu', {
		duration: 1,
		y: -window.innerHeight,
		opacity: 0,
		display: 'none'
	})
})
